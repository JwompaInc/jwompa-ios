...
class JTNC: BaseViewController,UITextViewDelegate {

    var arrayJTNC : NSMutableArray = NSMutableArray()
    ...
    override func viewDidLoad() {
        ...
        let strFunctionName = Constants().kTerms
        ...
        WebService.callGetServicewithStringPerameters(StringPerameter: "", FunctionName: strFunctionName, succes: { (responseObject:NSMutableDictionary, operation:AFHTTPRequestOperation) in
            ...
            if(responseObject.object(forKey: "user_data") != nil){
                ...
                self.arrayJTNC = ((responseObject.object(forKey: "user_data") as! NSArray).mutableCopy() as? NSMutableArray)!
                var str : String = String()
                str = ((self.arrayJTNC.object(at: 0) as AnyObject).value(forKey: "description") as? String)!
    ...
    func setMyView(){
        ...
        JImage.shareInstance().SetLabelOnUpperViewWithNormalFont(CGRect(x: 60, y: 25, width: screenWidth - 120, height: 30), nameOfString: "TERMS & CONDITIONS")
        
        
        JImage.shareInstance().setBackButton(CGRect(x: 15,y: 30,width: 20,height: 20))
        buttonBack.addTarget(self, action: #selector(JTNC.BackButtonTapped(_:)), for: UIControlEvents.touchUpInside)
